...
Public Class ParserDLL
    ...
    Class ClientConfig
        ...
        Private _splits() As Split = Nothing
        ...
        Public Property Splits As Split()
            Get
                Return _splits
            End Get
            Set(value As Split())
    ...
    Class Input
        ...
        Private _inputfieldElements() As FieldElement
        ...
        Public Property FieldElements As FieldElement()
            Get
                Return _inputfieldElements
            End Get
            Set(value As FieldElement())
    ...
    Public Sub CreateClientProfile(ByVal clientConfig As ClientConfig)
        ...
        Using writer As XmlWriter = XmlWriter.Create("Test.xml", settings)
            ...
            For Each fieldElement As FieldElement In clientConfig.Input.FieldElements
    ...
    Public Function LoadClientProfile(ByVal xmlClientProfile As String) As ClientConfig
        Try
            ...
            For Each xDE As XmlNode In xmldoc.DocumentElement
                Select Case xDE.Name
                    Case GetEnumDesc(Xml_ElementName.Input)
                        PopulateObjectPerNode(xDE, input)
                    ...
                    Case GetEnumDesc(Xml_ElementName.Concatenate)
                        ...
                        PopulateObjectPerNode(xDE, concatenate, concatenates)
                        'concatenates.Add(concatenate)
                    Case GetEnumDesc(Xml_ElementName.Replace)
                        Dim replace As New Replace
                        PopulateObjectPerNode(xDE, replace, replaces)
                        'replaces.Add(replace)
                    Case GetEnumDesc(Xml_ElementName.Split)
                        Dim split As New Split
                        PopulateObjectPerNode(xDE, split, splits)
                        'splits.Add(split)
                    Case GetEnumDesc(Xml_ElementName.Exclusion)
                        Dim exclusion As New Exclusion
                        PopulateObjectPerNode(xDE, exclusion, exclusions)
            ...
            clientConfig.Splits = splits.ToArray
            ...
            For Each inputfieldElement As FieldElement In clientConfig.Input.FieldElements
    ...
    Private Sub PopulateObjectPerNode(ByVal xmlNode As XmlNode, ByRef obj As Object, Optional ByRef objs As Object = Nothing)
        Try
            Dim _fieldElements = Nothing
            If TypeOf obj Is Input Then
                _fieldElements = New List(Of FieldElement)
            ElseIf TypeOf obj Is Output Then
                _fieldElements = New List(Of OutputFieldElement)
            ElseIf TypeOf obj Is Replace Then
                _fieldElements = New List(Of ReplaceFieldElement)
            ElseIf TypeOf obj Is Concatenate Then
                _fieldElements = New List(Of ConcatenateFieldElement)
            ElseIf TypeOf obj Is Split Then
                _fieldElements = New List(Of SplitFieldElement)
            ElseIf TypeOf obj Is Exclusion Then
                _fieldElements = New List(Of ExclusionFieldElement)
            End If

            Dim type1 As Type = obj.GetType

            Dim piObj As System.Reflection.PropertyInfo = Nothing
            For Each xmlAttribute As XmlAttribute In xmlNode.Attributes
                piObj = type1.GetProperty(xmlAttribute.Name)
                Select Case piObj.PropertyType.Name
                    Case "Int16"
                        piObj.SetValue(obj, CShort(xmlAttribute.Value), Nothing)
                    Case "Int32", "Int64"
                        piObj.SetValue(obj, CInt(xmlAttribute.Value), Nothing)
                    Case "Boolean"
                        piObj.SetValue(obj, CBool(xmlAttribute.Value), Nothing)
                    Case "Date", "DateTime"
                        piObj.SetValue(obj, CDate(xmlAttribute.Value), Nothing)
                    Case "ParserType"
                        piObj.SetValue(obj, DirectCast([Enum].Parse(GetType(ParserType), xmlAttribute.Value), Integer), Nothing)
                    Case Else
                        piObj.SetValue(obj, xmlAttribute.Value, Nothing)
                End Select
            Next

            For Each _xmlChildNode As XmlNode In xmlNode.ChildNodes
                Dim _fieldElement = Nothing
                If TypeOf obj Is Input Then
                    _fieldElement = New FieldElement
                ElseIf TypeOf obj Is Output Then
                    _fieldElement = New OutputFieldElement
                ElseIf TypeOf obj Is Replace Then
                    _fieldElement = New ReplaceFieldElement
                ElseIf TypeOf obj Is Concatenate Then
                    _fieldElement = New ConcatenateFieldElement
                ElseIf TypeOf obj Is Split Then
                    _fieldElement = New SplitFieldElement
                ElseIf TypeOf obj Is Exclusion Then
                    _fieldElement = New ExclusionFieldElement
                End If

                Dim type2 As Type = _fieldElement.GetType

                Select Case _xmlChildNode.Name
                    Case GetEnumDesc(Xml_ElementName.Column)
                        For Each xmlAttribute As XmlAttribute In _xmlChildNode.Attributes
                            piObj = type2.GetProperty(xmlAttribute.Name)
                            Select Case piObj.PropertyType.Name
                                Case "Int16"
                                    piObj.SetValue(_fieldElement, CShort(xmlAttribute.Value), Nothing)
                                Case "Int32", "Int64"
                                    piObj.SetValue(_fieldElement, CInt(xmlAttribute.Value), Nothing)
                                Case "Boolean"
                                    piObj.SetValue(_fieldElement, CBool(xmlAttribute.Value), Nothing)
                                Case "Date", "DateTime"
                                    piObj.SetValue(_fieldElement, CDate(xmlAttribute.Value), Nothing)
                                Case Else
                                    piObj.SetValue(_fieldElement, xmlAttribute.Value, Nothing)
                            End Select
                        Next
                End Select
            ...
            If Not _fieldElements Is Nothing Then obj.FieldElements = _fieldElements.ToArray
            If Not objs Is Nothing Then objs.Add(obj)
        Catch ex As Exception
            SaveToLog("PopulateObjectPerNode(): Runtime error catched " & ex.Message)
        End Try
    End Sub
    ...
    Private Function PopulateConfigPerNode(ByVal xmlNode As XmlNode) As Object
        Try
            ...
            End If
        Catch ex As Exception
            SaveToLog("PopulateConfigPerNode(): " & ex.Message)
        End Try
    End Function
        ...
        Try
            ...
            For Each colData As DataColumn In _dtInputData.Columns
                ...
                If colData.ColumnName <> "RecordID" Then
                    Dim response
                    Dim _fieldElement = GetInputColumnAttribute(clientConfig.Input, colData.ColumnName)
                    Dim value As String = ""
                    ...
                    If _fieldElement.Replace <> "" Then
                        response = GetObjectByID(_clientConfig.Replaces, _fieldElement.Replace)
                        If Not response Is Nothing Then value = ReplaceValue(response, value)
                    End If

                    If _fieldElement.Concatenate <> "" Then
                        response = GetObjectByID(_clientConfig.Concatenates, _fieldElement.Concatenate)
                        If Not response Is Nothing Then value = ConcatenatedValue(response, rwData)
                    End If


                    If _fieldElement.Split <> "" Then
                        response = GetObjectByID(_clientConfig.Splits, _fieldElement.Split)
                        If Not response Is Nothing Then value = SplitValue(response, rwData)
                    End If
    ...
    Private Sub GenerateOutputTable(ByVal _output As Output, ByVal dtData As DataTable, ByRef intCntr As Integer,
        ...
        For Each rw As DataRowView In dtData.DefaultView
            ...
            For Each outputFieldElements As OutputFieldElement In _output.OutputFieldElements
                ...
                If _fieldElement.Split <> "" Then
                    response = GetObjectByID(_clientConfig.Splits, _fieldElement.Split)
                    If Not response Is Nothing Then _value = SplitValue(response, rw.Row)
                End If
    ...
    Private Overloads Function ConcatenatedValue(ByVal concatenate As Concatenate, ByVal rw As DataRow) As String
        ...
        Try
            For Each fieldElement As ConcatenateFieldElement In concatenate.FieldElements
                ...
                ElseIf fieldElement.Left <> "" Then
                    sbResult.Append(Microsoft.VisualBasic.Left(rw(fieldElement.RefID).ToString.Trim, CInt(fieldElement.Left)))
                ElseIf fieldElement.Right <> "" Then
                    sbResult.Append(Microsoft.VisualBasic.Right(rw(fieldElement.RefID).ToString.Trim, CInt(fieldElement.Right)))
                ElseIf fieldElement.SubStr = "" And fieldElement.DefaultValue = "" Then
                    sbResult.Append(rw(fieldElement.RefID).ToString.TrimEnd)
                ElseIf fieldElement.SubStr <> "" Then
                    If Not fieldElement.SubStr.Contains(",") Then
                        sbResult.Append(rw(fieldElement.RefID).ToString.Trim.Substring(fieldElement.SubStr).TrimEnd)
                    Else
                        sbResult.Append(rw(fieldElement.RefID).ToString.Trim.Substring(fieldElement.SubStr.Split(",")(0), fieldElement.SubStr.Split(",")(1)).TrimEnd)
                    End If
                End If
            Next
        Catch ex As Exception
        ...
        Return System.Text.RegularExpressions.Regex.Replace(sbResult.ToString, " {2,}", " ")
    End Function

    Private Overloads Function SplitValue(ByVal split As Split, ByVal rw As DataRow) As String
        Dim sbResult As New System.Text.StringBuilder
        Dim fieldID As String = ""

        Try
            For Each fieldElement As SplitFieldElement In split.FieldElements
                fieldID = fieldElement.ID

                If fieldElement.Delimiter = "" And fieldElement.DefaultValue <> "" Then
                    sbResult.Append(fieldElement.DefaultValue)
                ElseIf fieldElement.Delimiter <> "" Then
                    sbResult.Append(rw(fieldElement.RefID).Trim.ToString.Split(fieldElement.Delimiter)(fieldElement.IndexPosition).TrimEnd)
                End If
            Next
        Catch ex As Exception
            sbLog.AppendLine("SplitValue(): Failed in ID " & fieldID & ". Runtime error catched " & ex.Message)
        End Try
    ...
    Private Overloads Function ReplaceValue(ByVal replace As Replace, ByVal rw As DataRow) As String
        ...
        Try
            For Each fieldElement As ReplaceFieldElement In replace.FieldElements
                fieldID = fieldElement.ID
                sbResult.Append(rw(fieldElement.RefID).ToString.Trim.Replace(fieldElement.OldValue, fieldElement.NewValue).TrimEnd)
            Next
        Catch ex As Exception
    ...
    Private Overloads Function ReplaceValue(ByVal replace As Replace, ByVal value As String) As String
        ...
        Try
            For Each fieldElement As ReplaceFieldElement In replace.FieldElements
                fieldID = fieldElement.ID
                sbResult.Append(value.Trim.Replace(fieldElement.OldValue, fieldElement.NewValue).TrimEnd)
            Next
        Catch ex As Exception
            sbLog.AppendLine("ReplaceValue(): Failed in ID " & fieldID & ". Runtime error catched " & ex.Message)
        End Try
...
#Region " Misc "
    ...
    Private Function GetFieldValue(ByVal output As Output, ByVal ID As String, ByVal rw As DataRow, ByVal fileName As String) As String
        Dim _fieldElementInput = GetInputColumnAttribute(_clientConfig.Input, ID)
    ...
    '    Return clientConfig.Input.FieldElements.OfType(Of FieldElement)().Where(Function(element) element.ID.Contains(ID))(0)
    'End Function

    Private Function GetInputColumnAttribute(ByVal input As Input, ByVal ID As String) As FieldElement
        Return input.FieldElements.OfType(Of FieldElement)().Where(Function(element) element.ID.Equals(ID))(0)
    End Function

    Private Function GetOutputColumnAttribute(ByVal output As Output, ByVal ID As String) As OutputFieldElement
        Return output.OutputFieldElements.OfType(Of OutputFieldElement)().Where(Function(element) element.ID.Equals(ID))(0)
    End Function

    Private Function GetPDFOutputByID(ByVal pdfOutputs() As PDFOutput, ByVal ID As String) As PDFOutput
        Return pdfOutputs.OfType(Of PDFOutput)().Where(Function(element) element.ID.Equals(ID))(0)
    End Function

    Private Function GetConcatenateByID(ByVal concatenates() As Concatenate, ByVal ID As String) As Concatenate
        Return concatenates.OfType(Of Concatenate)().Where(Function(element) element.ID.Equals(ID))(0)
    End Function

    Private Function GetObjectByID(ByVal objects() As Object, ByVal ID As String) As Object
        Return objects.OfType(Of Object)().Where(Function(element) element.ID.Equals(ID))(0)
    End Function

    Private Function GetReplaceByID(ByVal replaces() As Replace, ByVal ID As String) As Replace
        Return replaces.OfType(Of Replace)().Where(Function(element) element.ID.Equals(ID))(0)
    End Function
